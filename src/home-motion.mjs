...
export const particleBenefit=index=>{const [name,heading,copy]=benefits[index];return `<section class="particle-benefit ${index%2?'particle-benefit-reverse':''}" data-light-scene="${name}" aria-labelledby="benefit-${name}">${lightField()}<div class="particle-benefit-layout wrap"><div class="particle-benefit-copy"><p class="eyebrow">${String(index+1).padStart(2,'0')} / 06 · YOUR £35 WEBSITE PLAN</p><h2 id="benefit-${name}">${heading}</h2><p>${copy}</p></div><div class="particle-icon-space" data-particle-anchor aria-hidden="true"></div></div></section>`;};
